...
import jwt from 'jsonwebtoken'; // Asegúrate de importar jwt si authenticateJWT lo usa
...
const toPublicProject = (project: Project, members: Partial<User>[] = []) => {
    const projectMembers = members.length > 0 ? members.filter(m => m.userId) : project.members;
    return {
...
router.post('/', authenticateJWT, async (req: AuthRequest, res: Response) => {
    try {
        ...
        await NotificationRepository.createNotification({
            ...
            message: `Has creado un nuevo proyecto: "${newProject.name}".`,
            type: NotificationType.PROJECT_CREATED,
        });
        } catch (notificationError) {
            console.error('Error al enviar notificación de creación de proyecto:', notificationError);
        }
    } catch (error) {
...
router.get('/:id', authenticateJWT, async (req: AuthRequest, res: Response) => {
    try {
        ...
        membersData.forEach(m => delete (m as any).passwordHash); // Usar passwordHash
...
router.put('/:id', authenticateJWT, async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (!updatedProject) {
            return res.status(500).json({ message: 'Error al obtener el proyecto actualizado.' });
        }
        ...
        try {
            ...
            console.error('Error al enviar notificación de actualización de proyecto:', notificationError);
        }
...
router.delete('/:id', authenticateJWT, async (req: AuthRequest, res: Response) => {
    try {
        ...
        try {
            await NotificationRepository.createNotification({
                userId: req.user!.userId,
                message: `Has eliminado el proyecto "${project.name}".`,
                type: NotificationType.PROJECT_UPDATED,
            });
        } catch (notificationError) {
            console.error('Error al enviar notificación de eliminación de proyecto:', notificationError);
        }
